...
class PEEPClient(StackingProtocol):
    ...
    async def data_timeout(self):
        print("Client: Inside Data Timer")
            ...
            for each_packet in packets:
                ...
                if self.global_received_ack <= self.global_number_seq:
                    if each_packet.packet.SequenceNumber == self.global_received_ack:
                        ...
                        #each_packet.flag += 1
                        print("Client: Packet Retransmitted.", each_packet.packet.SequenceNumber)
        ...
        for packet in self.deserializer.nextPackets():
            ...
            if self._state == 1 and packet.Type == 1:
                ...
                if checkvalue:
                    ...
                    self.RIP_seq_number = packet.SequenceNumber
                    print("RIP Received from Server with Seq. No.:", packet.SequenceNumber)
                ...
                if checkvalue:
                    ...
                    for chabi in seqs:
                        if packet.Acknowledgement > chabi:
                            (self.t[chabi]).cancel()
                            self.t.pop(chabi)
                    #print("Current active timers after receiving RIP-ACK: ", self.t, self.tz)
                    #self.pop_sending_window(packet.Acknowledgement)
                    self.exc = 0
                    print("Lost 1")
                    #self.tz.cancel()
    ...
    def encapsulating_packet(self,data_from_BL_1):
        ...
        if chunk == b'rip':
            ...
            ripbites = self.rip.__serialize__() #
            if self.rip_received == 0:
                print("Sent RIP to server with Seq. No.:", self.rip.SequenceNumber, "Ack. No.: None")
    ...
    def pop_sending_window(self, AckNum):
        print ("Client's pop sending window is called")
        self.AckNum = AckNum
        self.keylist = []
        self.keylist = list(self.sending_window)
        self.keylist1 = sorted(self.keylist)
        ...
        #print ("Client global number ack",self.global_number_ack)
        #print ("RIP Sequence number",self.RIP_seq_number)

        if self.sending_window_count == 1 and self.rip_received == 1 and self.backlog_window == [] and self.global_number_ack == self.RIP_seq_number:
                         #and self.global_number_ack == self.RIP_PKT.SequenceNumber + 50:
                         #self.sendack(self.global_number_ack)
                         self.sending_ripack(self.RIP_PACKET)

        if self.sending_window_count == 1 and self.backlog_window == [] and self.ripack_received == 1:
             self.close_timers()
             self._state += 1
             self.exc = 0
             seqs = list(self.t.keys())
             ...
             print("Lost 2")
             self.connection_lost(self.exc)
        #if self.sending_window_count == 0 and self.FLAG_CLOSE == 1 and self.rip_received == 0:
            #Data = b'rip'
            #self.write(Data)
    ...
    def sending_ripack(self, RIP_PKT):
        ...
        print("RIP-ACK sent from client to server. Ack. No.:", ripack.Acknowledgement)
        # asyncio.sleep(10)
        print("Lost 3")
    ...
    def close(self):
        self.FLAG_CLOSE = 1
        print ("client rip rec flag",self.rip_received)
        if self.rip_received == 0:
            Data = b'rip'
            self.write(Data)
    ...
    async def connection_timeout(self):
        ...
        while self.sending_window_count >= 0:
            ...
            if len(self.keylist1) == 1 & self.flags < 1:
                await asyncio.sleep(0.2)
                Data = b'rip'
                ripbites = self.rip.__serialize__() # :P
                print(" Retransmitting RIP ")
                self.transport.write(ripbites)
                self.flags = 1
...
class Timerx():
    ...
    def cancel(self):
        print("Cancelling the timer object for", self.packet)
        self._task.cancel()
