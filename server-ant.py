...
class PEEPServerProtocol(StackingProtocol):
    ...
    def connection_made(self, transport):
        print("\n================== PEEP Server Connection_made Called12123123131312312312++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++ =========================")
        self.transport = transport
        ...
        for pkt in self.deserializer.nextPackets():
            ...
            # Reset packet received
                ...
                if checkvalue:
                    self.rip_received = 1
                    self.RIP_PACKET = pkt
                    self.RIP_seq_num = pkt.SequenceNumber
    ...
    def sending_ripack(self, RIP_PKT):
        ...
        print("Sent RIP-ACK to client with Ack. No:", ripack.Acknowledgement)
        print("Current active timers after sending RIP-ACK: ", self.t)
        #asyncio.sleep(10)
        #self.connection_lost(self)


    def pop_sending_window(self, AckNum):
        print ("Server's pop sending window is called")
        self.AckNum = AckNum
        self.keylist = []
        ...
        print ("Server: Global number ACK",self.global_number_ack)

        if self.sending_window_count == 0 and self.rip_received == 1 and self.backlog_window == [] and self.global_number_ack == self.RIP_seq_num:
                         #and self.global_number_ack == self.RIP_PKT.SequenceNumber + 50:
                         #self.sendack(self.global_number_ack)
                         self.sending_ripack(self.RIP_PACKET)

        if self.sending_window_count == 0 and self.backlog_window == [] and self.ripack_received == 1:
            seqs = list(self.t.keys())
            for chabi in seqs:
                if self.AckNum > chabi:
                    (self.t[chabi]).cancel()
    ...
    def encapsulating_packet(self, data_from_BL_1):
        ...
        if chunk == b'rip':
                ...
                if self.rip_received == 0:
                    print("Sent RIP to client with Seq. No.:", self.rip.SequenceNumber, "Ack. No.: None")
    ...
    def close(self):
        self.FLAG_CLOSE = 1
        print ("server rip rec flag",self.rip_received)
        if self.rip_received == 0:
            Data = b'rip'
            self.write(Data)
